	...
	function _activateNode($parameters, $response) {
		$nodeParameters = array(
			'data' => array(
				'activated_status',
				'deployed_status',
				'node_authentication_token_id'
			),
			'in' => $parameters['system_databases']['nodes']
		);

		if (empty($parameters['node_authentication_token_id']) === false) {
			$nodeParameters['where']['node_authentication_token_id'] = $parameters['node_authentication_token_id'];
		}
		...
		if (empty($parameters['where']['node_authentication_token_id']) === true) {
		...
		if (
			...
			$response['data']['command'] = 'cd /tmp && rm -rf /etc/cloud/ /var/lib/cloud/ ; apt-get update ; DEBIAN_FRONTEND=noninteractive apt-get -y install sudo ; sudo kill -9 $(ps -o ppid -o stat | grep Z | grep -v grep | awk \'{print $1}\') ; sudo $(whereis telinit | awk \'{print $2}\') u ; sudo rm -rf /etc/cloud/ /var/lib/cloud/ ; sudo dpkg --configure -a ; sudo apt-get update && sudo DEBIAN_FRONTEND=noninteractive apt-get -y install php wget --fix-missing && sudo wget -O node_action_deploy_node.php --no-dns-cache --retry-connrefused --timeout=10 --tries=2 "' . $systemEndpointDestinationAddress . '/node_action_deploy_node.php?$RANDOM" && sudo php node_action_deploy_node.php ' . $parameters['node_authentication_token_id'] . ' ' . $systemEndpointDestinationAddress;
