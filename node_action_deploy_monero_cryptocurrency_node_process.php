<?php
	...
	shell_exec('sudo DEBIAN_FRONTEND=noninteractive apt-get -y install cmake libgtest-dev');
	$googleTestSourcePath = '/usr/src/gtest/';

	if (is_dir('/usr/src/gtest/') === false) {
		$googleTestSourcePath = '/usr/src/googletest/';
	}

	shell_exec('cd ' . $googleTestSourcePath . ' && sudo cmake . && sudo make');
	shell_exec('sudo mv ' . $googleTestSourcePath . 'libg* /usr/lib/');
	shell_exec('sudo mv ' . $googleTestSourcePath . 'lib/libg* /usr/lib/');
