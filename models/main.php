<?php
	...
	class MainModel extends Configuration {
		...
		protected function _decodeItems($parameters, $fetchItems = false) {
			$response = array();

			if (!empty($parameters['items'])) {
				foreach ($parameters['items'] as $itemListName => $items) {
					$response[$itemListName] = array_merge($items, array(
						'count' => isset($items['data']) ? count($items['data']) : 0,
						'encoded_items' => $parameters['data'],
						'name' => $itemListName
					));
					...
					$response[$itemListName]['from'] = $items['from'];
					$itemKeyLines = $items['data'];
					$itemKeyLineKeys = implode('', array_keys($itemKeyLines));
					$itemKey = $itemCount = 0;

					if (!is_numeric($itemKeyLineKeys)) {
						continue;
					}

					$itemKeys = array();

					foreach ($itemKeyLines as $itemKeyLineKey => $itemKeyLine) {
						$itemKeyLineParts = explode('_', $itemKeyLine);

						foreach ($itemKeyLineParts as $itemKeyLinePart) {
							$itemStatus = substr($itemKeyLinePart, 0, 1);
							$itemStatusCount = substr($itemKeyLinePart, 1);

							if ($itemStatus) {
								if ($fetchItems) {
									for ($i = 0; $i < $itemStatusCount; $i++) {
										$itemKeys[$itemKey + $i] = 1;
									}
								}
					...
					if ($fetchItems) {
						$itemParameters = $itemIdParameters = array(
							'fields' => array(
								'id'
							),
							'from' => $items['from'],
							'limit' => $itemKey,
							'offset' => ($itemKeyLineKey * 10000),
						);

						if (!empty($items['token']['parameters']['sort'])) {
							$itemIdParameters['sort'] = $items['token']['parameters']['sort'];
						}

						if (!empty($items['token']['parameters']['where'])) {
							$itemIdParameters['where'] = $items['token']['parameters']['where'];
						}

						if (!empty($parameters['search'][$itemListName])) {
							$itemIdParameters['where']['_search'] = $parameters['search'][$itemListName];
						}

						$itemIds = $this->fetch($itemIdParameters);
						...
						if (!empty($itemIds['data'])) {
							$where['id'] = array_values(array_intersect_key($itemIds['data'], $itemKeys));
						}

						$response[$itemListName] = array_merge($response[$itemListName], $this->fetch(array(
							'fields' => array(
								'id'
							),
							'from' => $items['from'],
			...
			if (!empty($parameters['items'])) {
				foreach ($parameters['items'] as $itemListName => $items) {
					...
					if (!empty($itemIds['count'])) {
						$selectedItemKeys = array_intersect($itemIds['data'], $items['data']);
						$unselectedItemKeys = array_diff($itemIds['data'], $items['data']);
						array_walk($selectedItemKeys, function(&$selectedItemIdValue, $selectedItemIdKey) {
							$selectedItemIdValue = 1;
						});
						array_walk($unselectedItemKeys, function(&$unselectedItemIdValue, $unselectedItemIdKey) {
							$unselectedItemIdValue = 0;
						});
						$allItemKeys = $selectedItemKeys + $unselectedItemKeys;
						ksort($allItemKeys);
						$itemKeyLineKey = $itemStatusCount = 0;
						$itemKeys = array();

						foreach ($allItemKeys as $itemKey => $itemStatus) {
							if (((10001 * ($itemKeyLineKey + 1)) - $itemKey) === 1) {
								$itemKeys[$itemKeyLineKey][] = $itemStatus . $itemStatusCount;
								$itemStatusCount = 0;
								$itemKeyLineKey++;
							}
							...
							if (
								!isset($allItemKeys[$itemKey + 1]) ||
								$allItemKeys[$itemKey + 1] != $itemStatus
							) {
								$itemKeys[$itemKeyLineKey][] = $itemStatus . $itemStatusCount;
								$itemStatusCount = 0;
							}
						...
						foreach ($itemKeys as $itemKeyLineKey => $itemStatusCounts) {
							$itemKeys[$itemKeyLineKey] = implode('_', $itemStatusCounts);
						}
					}

					$response['items'][$itemListName] = array_merge($parameters['items'][$itemListName], array(
						'count' => count($selectedItemKeys),
						'data' => $itemKeys
					));
				}
			}
		...
		protected function _formatQuery($from, $where = array(), $conjunction = 'OR') {
			...
			foreach ($where as $key => $value) {
				...
				if (
					...
					$where[$key] = '(' . implode(') ' . $conjunction . ' (', $this->_formatQuery($from, $value, $conjunction)) . ')';
				} else {
					if (is_array($value)) {
						array_walk($value, function(&$fieldValue, $fieldKey) use ($key, $operators) {
							$key = (strlen($fieldKey) > 1 && is_string($fieldKey) ? $fieldKey : $key);
							$fieldValue = (is_null($fieldValue) ? $key . ' IS NULL' : trim(in_array(trim(substr($key, strpos($key, ' '))), $operators) ? $key : $key . ' =') . ' ' . $this->_prepareValue($fieldValue));
						});
					} else {
						$value = array((is_null($value) ? $key . ' IS NULL' : trim(in_array(trim(substr($key, strpos($key, ' '))), $operators) ? $key : $key . ' =') . ' ' . $this->_prepareValue($value)));
					}
		...
		protected function _parameterizeSQL($query) {
			...
			foreach ($queryParts as $queryPartKey => $queryPart) {
				if (
					($position = strpos($queryPart, $this->keys['stop'])) !== false &&
					$queryPart = str_replace($this->keys['stop'], '?', $queryPart)
				) {
					$queryParts[$queryPartKey] = str_replace(($between = substr($queryPart, 0, $position)), '', $queryPart);
		...
		protected function _parseItemListTokenParameters($itemListTokenParameters) {
			$response = array(
				'foreign_key' => $itemListTokenParameters['foreign_key'],
				'foreign_value' => $itemListTokenParameters['foreign_value'],
				'parameters_to_encode' => $itemListTokenParameters['parameters']
			);
			return $response;
		}
		...
		public function delete($parameters) {
			...
			if (
				...
				$query .= ' WHERE ' . ($queryData = implode(' AND ', array_filter($this->_formatQuery($parameters['from'], $parameters['where']))));
